...
//  
//
//  Created by Shotaro Tsubokawa on 2020/07/06.
//
...
void display(void) {
  ...
  //
    ...
    glRotatef(g_angle[0], 1, 0, 0);
    glRotatef(g_angle[1], 0, 1, 0);
    glRotatef(g_angle[2], 0, 0, 1);
...
void rotate(int ax, GLfloat amt) {
  // Complete
    g_angle[ax] = g_angle[ax] + amt;
    if(g_angle[ax] > 360) g_angle[ax] =  g_angle[ax] - 360;
